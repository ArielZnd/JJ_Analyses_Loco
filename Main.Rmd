...
# plotpath <- "E:/Documents/ProjetsStreetlab/JnJ/J_n_J_WO6_Artemis2(Dec2022- )/Plot/"
plotpath <- "D:/Streetlab/JJ/J_n_J_WO6_Artemis2(Dec2022- )/Plot"
...
```{r Crossing Normality, eval=FALSE}
SaveFig <- "TRUE"
...
```{r Crossing Distribution for each subject, eval=FALSE}
...
### Plot for all subj
```{r Crossing Distribution all subj, eval=FALSE }
list.subj <- unique(df$SubjID)
for (var in my_vars) {
    print(paste("Boxplot variable : ",var))
...
t_test_results <- list()
# Boucle à travers les variables (à partir de la colonne 2, car la colonne 1 est subjectID)
for (variable in my_vars) {
    # Effectuer le test t pour la variable actuelle
  t_test_result <- t.test(df[[variable]] ~ df$IOL)
...
for (var in variables_non_normales) {
...
```{r Complex Load data, eval=FALSE}
df <- data.frame()
...
```
## Normality test - Shapiro
```{r Complex Normality, eval=FALSE}
SaveFig <- "TRUE"
variables_normales <- c()
variables_non_normales <- c()
...
## Visualisation des données
```{r Complex - Distribution Boxplots, eval=FALSE}
for (var in my_vars) {
  print(paste("Boxplot variable : ",var))
